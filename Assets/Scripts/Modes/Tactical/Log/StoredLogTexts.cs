...
static class StoredLogTexts
{
    ...
    internal static void InitializeLists()
    {
        ...
        SwallowMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"Finding a chance, <b>{i.Unit.Name}</b> gives in to {GPPHis(i.Unit)} hunger and snatches up an unfamiliar {GetRaceDescSingl(i.Target)}, savouring <b>{i.Target.Name}</b> for a moment before {GetRandomStringFrom("eating", "devouring", "voring", "gobbling down", "swallowing down", "gulping down", "slurping down", "wolfing down")} {GPPHis(i.Unit)} {GetPreyDesc(i.Target)} prey.",
        ...
        RandomDigestionMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Target.Name}</b> struggles inside <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {i.preyLocation.ToSyn()}, trying {GPPHis(i.Unit)} {GetRandomStringFrom("best", "hardest", "damndest")} to avoid becoming fresh {GetRandomStringFrom("shit", "waste", "dung", "scat", "crap", "manure", "turds")}.",
        ...
        BellyRubMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Target.Name}</b> nervously asks <b>{i.Unit.Name}</b> for help as {GPPHis(i.Target)}'s body makes a weird squelching noise, the <b>{i.Unit.Name}</b> laughs and pats the newbie's {GetRandomStringFrom("sloshing", "sloshy", "gurgling", "gurgly", "churning", "groaning", "growling", "rumbling", "rumbly", "hungry", "hungering", "ravenous", "voracious", "gluttonous", "wailing", "shifting", "trembling", "wobbling", "stuffed", "full", "bloated", "bulging", "thrashing", "bubbling", "bubbly", "pulsating", "curvaceous", "prey-filled", "prey-stuffed", "rounded", "soft")} {PreyLocStrings.ToSyn(PreyLocation.stomach)}.",
        ...
        BallMassageMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"<b>{i.Target.Name}</b> is about to rub {GPPHis(i.Target)} own {PreyLocStrings.ToSyn(PreyLocation.balls)}, when <b>{i.Unit.Name}</b> quickly claims the spot and starts grinding against {GPPHis(i.Target)} crotch. The two lions make out, and this evolves into mutual dry humping, threatening to liquefy any genital captives as the sex juices start flowing.",
            priority: 16, conditional: s => s.Target != s.Unit ,targetRace: Race.FeralLions, actorRace: Race.FeralLions),
            new EventString((i) => $"<b>{i.Unit.Name}</b> nonchalantly sits down on {GPPHis(i.Target)} {PreyLocStrings.ToSyn(PreyLocation.balls)}, compressing <b>{i.Prey.Name}</b> together with all the spunk.",
            priority: 16, conditional: s => s.Target == s.Unit ,targetRace: Race.FeralLions, actorRace: Race.FeralLions),
            ...
            new EventString((i) =>$"<b>{i.Unit.Name}</b> gives a gentle kiss to <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {PreyLocStrings.CockAdjSyn()} {PreyLocStrings.ToCockSyn()}, giggling at {GPPHis(i.Unit)} muffled gasp of pleasure.", actorRace: Race.Succubi, priority: 9),
            ...
            new EventString((i) =>$"<b>{i.Unit.Name}</b> massages {(i.Unit == i.Target ? GPPHis(i.Target) : "<b>" + i.Target.Name + "</b>'s")} full scrotum, a bit of pre leaking out of {GPPHis(i.Target)} {PreyLocStrings.CockAdjSyn()} {PreyLocStrings.ToCockSyn()}.", priority: 8),
            ...
            new EventString((i) =>$"Dropping to the ground, <b>{i.Unit.Name}</b> spreads {GPPHis(i.Unit)} legs and rubs {GPPHis(i.Unit)} thighs against {GPPHis(i.Unit)} {PreyLocStrings.CockAdjSyn()} {PreyLocStrings.ToCockSyn()}, enjoying the deep burbling within.", priority: 11, conditional:s => s.Target == s.Unit),
        ...
        AbsorptionMessages = new List<EventString>()
        {
            ...
            new EventString((i) => $"With a squint and a push from <b>{i.Unit.Name}</b>, <b>{i.Target.Name}</b> manages to achieve {GPPHis(i.Target)} final form as {InfoPanel.RaceSingular(i.Unit)} {GetRandomStringFrom("shit", "waste", "dung", "scat", "crap", "manure", "excrement", "droppings")}.", priority: 9, conditional: Scat),
            new EventString((i) => $"<b>{i.Unit.Name}</b> grunts, and the {GetRandomStringFrom("nasty", "creamy", "warm", "greasy", "foul", "gross", "thick and brown", "fragrant", "steamy", "steaming", "heavy", "slimy", "freshly churned", "messy", "disgusting", "filthy", "stinky", "odorous", "fetid", "rank", "rancid", "vile", "putrid", "skunky")} remains of {GPPHis(i.Unit)} now used-up prey splat onto the ground behind {GPPHim(i.Unit)}. <b>{i.Target.Name}</b> is completely unrecognizable.", priority: 9, conditional: Scat),
            ...
            new EventString((i) => $"<b>{i.Unit.Name}</b>: \"Hnng...\". <b>{i.Target.Name}</b>: <i>~Splat~</i>", priority: 9, conditional: s => Scat(s)),
            new EventString((i) => $"A cacophonous fart out of <b>{ApostrophizeWithOrWithoutS(i.Unit.Name)}</b> {GetRandomStringFrom("ass", "asscheeks", "buns", "butt", "buttcheeks", "cheeks", "rear", "rear end", "hindquarters", "backside", "behind", "booty")} {GetRandomStringFrom("heralds", "announces", "signals", "preludes", "plays harbinger to", "plays forerunner to")} <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {GetRandomStringFrom("nasty", "creamy", "warm", "greasy", "foul", "gross", "thick and brown", "fragrant", "steamy", "steaming", "heavy", "slimy", "freshly churned", "messy", "disgusting", "filthy", "stinky", "odorous", "fetid", "rank", "rancid", "vile", "putrid", "skunky")} return to the battlefield...", priority: 9, conditional: s => Scat(s) && Farts(s)),
        ...
        {
            new EventString((i) => $"<b>{i.Unit.Name}</b> pumps the cum left of {GPPHis(i.Unit)} prey into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> mouth; {i.Target.Name} is happy with these {GetRandomStringFrom("creamy", "silky", "cummy", "jizzy", "tasty", "delicious", "lewd", "naughty", "sexy", "spoogey", "gooey", "thick")} results.",
            priority: 10, conditional: s => InStomach(s) && s.Prey.IsDead ),
            new EventString((i) => $"<b>{i.Unit.Name}</b> pumps the cum left of {GPPHis(i.Unit)} prey into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> mouth; {i.Target.Name} eagerly gulps down the {GetRandomStringFrom("creamy", "silky", "cummy", "jizzy", "tasty", "delicious", "lewd", "naughty", "sexy", "spoogey", "gooey", "thick")} treat.",
            priority: 10, conditional: s => InStomach(s) && s.Prey.IsDead ),
            new EventString((i) =>$"<b>{i.Unit.Name}</b> pumps <b>{i.Prey.Name}</b> into <b>{ApostrophizeWithOrWithoutS(i.Target.Name)}</b> {i.preyLocation.ToSyn()}; {i.Target.Name} is quick to swallow {GPPHim(i.Prey)} down.",
            priority: 10, conditional: s => InStomach(s) && !s.Prey.IsDead ),

            //Very lewd
            new EventString((i) => $"<b>{i.Unit.Name}</b> beckons <b>{i.Target.Name}</b> over and tells {GPPHim(i.Target)} to start sucking. <b>{i.Target.Name}</b> is just too good with {GPPHis(i.Target)} mouth and <b>{i.Unit.Name}</b> {GetRandomStringFrom("climaxes", "orgasms", "goes over the edge", "cums", "ejaculates", "nuts")} soon after, bloating out the {GetRaceDescSingl(i.Target)}'s belly as {GPPHis(i.Unit)} balls shrink down to normal size.",
            priority: 11, conditional: s => InStomach(s) && Lewd(s) && s.Prey.IsDead ),
            new EventString((i) => $"<b>{i.Unit.Name}</b> beckons <b>{i.Target.Name}</b> over and tells {GPPHim(i.Target)} to start sucking. <b>{i.Target.Name}</b> is just too good with {GPPHis(i.Target)} mouth and <b>{i.Unit.Name}</b> {GetRandomStringFrom("climaxes", "orgasms", "goes over the edge", "cums", "ejaculates", "nuts")} soon after, bloating out the <b>{i.Target.Name}</b>'s belly when <b>{i.Prey.Name}</b> shoots froms {GPPHis(i.Prey)} testicular prison and straight down the {GetRaceDescSingl(i.Target)} throat.",
            priority: 11, conditional: s => InStomach(s) && Lewd(s) && !s.Prey.IsDead ),
