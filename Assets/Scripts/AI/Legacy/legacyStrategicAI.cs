...
namespace LegacyAI
{
    public class LegacyStrategicAI : IStrategicAI
    {
        ...
        bool Sneak(Army army)
        {
            ...
            if (p != null)
            {
                path = StrategyPathfinder.GetPath(Empire, army.Position, p, army.RemainingMP, army.movementMode == Army.MovementMode.Flight);
                if (path == null)
                    army.RemainingMP = 0;
        ...
        bool ArmyReady(Army army)
        {
            ...
            else
            {
                ...
                {
                    path = StrategyPathfinder.GetPath(Empire, army.Position, village.Position, army.RemainingMP, army.movementMode == Army.MovementMode.Flight);
                    return false;
                }
            }
